package jp.ac.std.it_college.s22019.databasesample
...
import jp.ac.std.it_college.s22019.databasesample.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun initList(list: RecyclerView) {
        ...
        val adapter = CocktailAdapter(data.toList()) { pos, name ->
            ...
            // 選択されたカクテルのID（番号）をMainActivity のプロパティで保持
    ...
    private fun onSaveButton(view: View) {
        ...
        db.compileStatement(deleteSQL).let { stmt ->
            // プレースホルダ（？）に 変数の値(id)をバインド(割当)
            stmt.bindLong(1, _cocktailId)
            // 組み立てたDELETE　SQL　文を実行
